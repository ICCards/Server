		...
		match result[0]: #switch statement for first byte
			...
			1: #request for input received
				...
				for _x in range(packet_amount):
					Server.rpc_id(int(name),"input",packet_arr)
...
func _physics_process(_delta):
	if Input.is_key_pressed(KEY_ESCAPE):
		game = Game.END
		for _x in range(packet_amount):
			Server.rpc_id(int(name),"input",[3]) #send game end signal to networked game
	...
	if (input_received):
		...
		if state_queue[0].actual_input == false:
			if input_arrival_array[state_queue[0].frame] == false:
				...
				for _x in range(packet_amount):
					Server.rpc_id(int(name),"input",[1, state_queue[0].frame, (frame_num + input_delay)%256]) #send request for needed input
